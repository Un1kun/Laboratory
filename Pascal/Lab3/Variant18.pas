...
Begin
  ...
  begin
    ...
    GotoXY(15,s);     
    //Избавляемся от 10 цифр после запятой при n=3, просто вопрос красоты
    if i=3 then begin Writeln(a:2:2);
    end
    else Writeln(a);
    if D=0 then begin
    X1:=-b/(2*a);
    ...
    pr4:=a*sqr(x2)+b*x2;
      if D=0 then begin
        ...
        GotoXY(60,s); Writeln('Проверка при x1: ',pr1,'  Проверка при x2: ',pr2);
      end
      else if D<0 then begin
